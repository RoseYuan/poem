...
The package `r Rpackage("ClusteringMetrics")` includes many metrics to perform different kinds of evaluations, and these metrics can be retrieved via 6 main wrapper functions. Unless specified, "partition" means "hard" partition. They are:
...
plotGraphs <- function(d, k=7){
  gn <- dplyr::bind_rows(lapply(split(d[,-1],d$graph), FUN=function(d1){
    nn <- ClusteringMetrics:::.emb2knn(as.matrix(d1[,c("x","y")]), k=k)
    g <- ClusteringMetrics:::.nn2graph(nn, labels=d1$class)
