...
package: "`r BiocStyle::pkg_ver('ClusteringMetrics')`"
author: 
- name: Siyuan Luo
...
[^2]: Despite hard partitions where each data point belongs to one and only one subset, clustering can also generate fuzzy partitions. A fuzzy partition allows data points to belong to multiple subsets with varying degrees of membership. 
...
bind_rows(list(graph1=res1, graph2=res2), .id="graph") %>% pivot_longer(cols=c("meanSW","dbcv"), names_to="metric",values_to="value") %>%
...
For our examples `g1` and `g2`, the constucted graphs will look like:
...
Similarly, `level` can be `"element"`, `"class"` or `"global"`.

```{r}
getGraphMetrics(x=g1[,c("x","y")], labels=g1$class, metrics=c("PWC","ISI"), level="class", directed=FALSE, k=7, shared=FALSE)
```
...
bind_rows(list(graph1=res1, graph2=res2), .id="graph") %>% pivot_longer(cols=c("PWC","ISI"), names_to="metric",values_to="value") %>%
ggplot(aes(class, value, fill=graph, group=graph)) + 
  geom_bar(position = "dodge", stat = "identity") +
  facet_wrap(~metric) +
  theme_bw()
...
k <- 7
r <- 0.5
snn1 <- ClusteringMetrics:::.emb2snn(as.matrix(g1[,c("x","y")]), k=k)
snn2 <- ClusteringMetrics:::.emb2snn(as.matrix(g2[,c("x","y")]), k=k)
...
```{r}
# for g1
getPartitionMetrics(true=g1$class, pred=g1$cluster, level="global",
                    metrics = c("RI", "WC", "WH", "ARI", "AWC", "AWH",
                                "FM", "AMI"))

# for g2
getPartitionMetrics(true=g2$class, pred=g2$cluster, level="global",
                    metrics = c("RI", "WC", "WH", "ARI", "AWC", "AWH",
                                "FM", "AMI"))
```
...
getFuzzyPartitionMetrics(hardPred=hardPred, hardTrue=hardTrue, fuzzyTrue=fuzzyTrue, nperms=3, level="class", fuzzy_true = TRUE, fuzzy_pred = FALSE)
```
By specifying `fuzzy_true` and `fuzzy_pred`, one can control whether the fuzzy or hard version of the two partitions is used in comparison. This should be consistent with the input `hardPred`, `hardTrue`, `fuzzyPred`, `fuzzyTrue`, e.g. when `fuzzy_true=TRUE`, `fuzzyTrue` should not be `NULL`.
...
plot_grid(p0 + theme(plot.title = element_text(hjust = 0.5)),
          ...
          p2 + theme(plot.title = element_text(hjust = 0.5)), nrow=1, rel_width=c(1,1,1))
```
