...
Note that the indices in columns "class" and "cluster" correspond to the levels of original factors passed to `true` and `pred`. We align them back to the previous factor values, and then plot them in heatmap:
...
column_ha = HeatmapAnnotation(class=c("1", "2", "3", "4", "5", "6", "7", "8"),
                              col = list(class = my_cols),  
...
p4 <- awh %>%
  Heatmap(
    ...
    width = ncol(awh)*unit(6, "mm"), 
    height = nrow(awh)*unit(7, "mm"))

p4 <- grid.grabExpr({
 draw(p4, merge_legend = TRUE, padding = unit(c(12, 2, 2, 2), "mm"))
})
```
...
col_ls <- my_cols[1:7]
names(col_ls) <- c("WM", "Layer6", "Layer5", "Layer4", "Layer3", "Layer2", "Layer1")
column_ha = HeatmapAnnotation(class=c("WM", "Layer6", "Layer5", "Layer4", "Layer3", "Layer2", "Layer1"),
...
p5 <- awc %>%
  Heatmap(
    ...
    cluster_columns = FALSE, 
    cluster_rows = FALSE, 
    column_order = c("WM", "Layer6", "Layer5", "Layer4", "Layer3", "Layer2", "Layer1"),
    bottom_annotation = column_ha,
    width = ncol(awc)*unit(6, "mm"), 
    height = nrow(awc)*unit(7, "mm"))

p5 <- grid.grabExpr({
 draw(p5, merge_legend = TRUE)
})
```
