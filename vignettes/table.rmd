...
partition_metrics <- data.frame(
  ...
  Calculation = c(
    ...
    "A normalized version of RI, where each agreement quantities are divided by the maximum possible agreement values for their respective class.",
...
spatial_metrics <- data.frame(
  ...
  Calculation = c(
    ...
    "Spatial versions of the pair-sorting indices, based on fuzzy versions of the metrics. Specifically, we use the Normalized Degree of Concordance (NDC, see Hullermeier et al., 2012) and the Adjusted Concordance Index (ACI, see D'Errico et al., 2021) as fuzzy versions of RI and ARI respectively, and developed fuzzy versions of the other metrics using the same logic. In the spatial context, we first make a fuzzy version of the true labels based on the spatial neighborhood, and then track the maximum pair agreement between the predicted labels and either the hard or fuzzy ground truth.",
    "The proportion, for each spot, of its concordance with all other spots that is the same (i.e. concordant or not) across the clustering and ground truth. This value will be the same for all spots that share the same combination of cluster and class, and is especially useful for visualization. A variant of this can be computed that ignores negative pairs (i.e. that are discordant in both the clustering and ground truth). When negative pairs are included, the average of SPA equals to the Rand Index.",
    "Like the non-spatial Spot-wise Pair Agreement, with the difference that the clustering is evaluated against both a 'hard' and 'fuzzy' version of the ground truth, as for the computation of the Spatial versions of the pair-sorting indices.",
