...
test_that("Internal spatial metrics work", {
  sm <- getSpatialInternalMetrics(d$label, d[,1:2], level="dataset")
  expect_true(all(!is.na(as.matrix(sm))))
  sm <- getSpatialInternalMetrics(d$label, d[,1:2], level="class")
  sm2 <- getSpatialInternalMetrics(d$label, d[,1:2], level="element",
                                   metrics=c("PAS","ELSA"))
...
test_that("External spatial metrics work", {
  sm <- getSpatialExternalMetrics(d$label, d$p1, d[,1:2], level="dataset")
  expect_true(all(!is.na(as.matrix(sm))))
  sm <- getSpatialExternalMetrics(d$label, d$p1, d[,1:2], level="class")
  expect_true(sm$SpatialAWH[3]==1)
  expect_true(sm$SpatialAWC[2]==1)
  sm2 <- getSpatialExternalMetrics(d$label, d$p1, d[,1:2], level="element",
