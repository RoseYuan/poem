...
test_that("Embedding metrics work", {
  m1 <- getEmbeddingMetrics(d1[,1:2], d1$class, level="class", shared=TRUE)
  m1 <- getEmbeddingMetrics(d1[,1:2], d1$class, level="class")
  m2 <- getEmbeddingMetrics(d1[,1:2], d1$class, level="dataset", 
                            metrics=c("meanSW", "meanClassSW", "pnSW", "minClassSW", "cdbw", "cohesion", "compactness", "sep", "dbcv"))
  m3 <- getEmbeddingMetrics(d2[,1:2], d2$class, level="dataset",
                            metrics=c("meanSW", "meanClassSW", "pnSW", "minClassSW", "cdbw", "cohesion", "compactness", "sep", "dbcv"))
...
test_that("Dist input works", {
  m1 <- getEmbeddingMetrics(dist(d1[,1:2]), d1$class, level="class", shared=TRUE)
